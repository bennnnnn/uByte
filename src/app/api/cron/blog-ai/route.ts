 ...
 *
 * Security: requires Authorization: Bearer CRON_SECRET (same as other crons).
 *
 * Env (optional):
 *   BLOG_AI_CRON_ENABLED     — must be "1" or "true" or the handler no-ops (safe default).
 *   BLOG_AI_POSTS_PER_RUN    — posts to create this run (default 6, max 6).
...
export const GET = withErrorHandling("GET /api/cron/blog-ai", async (request: NextRequest) => {
  ...
  const wantPerRun = Math.min(6, Math.max(1, parseInt(process.env.BLOG_AI_POSTS_PER_RUN || "6", 10) || 6));
  ...
  for (const topic of topics) {
    const r = await generateAndInsertAiBlogPost(topic, takenSlugs);
    if (r.ok) {
