...
interface Props {
  experienceId: number;
  initialScore: number;
  initialVote?: 1 | -1 | 0 | null;
}

export default function VoteButton({ experienceId, initialScore, initialVote }: Props) {
  const [score, setScore] = useState(initialScore);
  // Track which direction the user has voted (null = not voted)
  ...
  const handleVote = async (direction: 1 | -1) => {
    ...
    // Clicking the active direction again = un-vote (send 0); otherwise vote in that direction
    const next: 0 | 1 | -1 = myVote === direction ? 0 : direction;
  ...
  return (
    <div className="mt-4 flex items-center gap-2">
      <button
        ...
        disabled={busy}
        aria-label={isUpvoted ? "Remove helpful vote" : "Mark as helpful"}
      ...
      >
        ...
        {score !== 0 && (
          <span className={`ml-0.5 rounded-full px-1.5 py-0.5 text-[10px] font-bold ${
            isUpvoted ? "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/60 dark:text-indigo-300" : "bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400"
          }`}>
            {score > 0 ? `+${score}` : score}
          </span>
        )}
