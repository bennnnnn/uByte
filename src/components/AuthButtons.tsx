...
export default function AuthButtons() {
  const { user, loading, logout, login, signup, profile, progress } = useAuth();
        ...
        {menuOpen && (
          <div className="absolute right-0 top-full z-50 mt-2 w-64 overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-lg dark:border-zinc-800 dark:bg-zinc-900">
              ...
              <div className="flex items-center justify-between">
                ...
                {profile && (
                  <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${
                    profile.plan === "yearly" || profile.plan === "pro"
            ...
            <div className="py-1">
              ...
              <Link href="/profile?tab=progress" onClick={() => setMenuOpen(false)}
              ...
              <Link href="/profile?tab=achievements" onClick={() => setMenuOpen(false)}
                className="flex items-center gap-3 px-4 py-2.5 text-sm text-zinc-700 transition-colors hover:bg-zinc-50 dark:text-zinc-300 dark:hover:bg-zinc-800">
              ...
              {allDone && user && (
                <Link href={`/certificate/${user.id}`} onClick={() => setMenuOpen(false)}
            ...
            <div className="border-t border-zinc-100 py-1 dark:border-zinc-800">
              ...
              <Link href="/profile?tab=plan" onClick={() => setMenuOpen(false)}
                className="flex items-center gap-3 px-4 py-2.5 text-sm text-zinc-700 transition-colors hover:bg-zinc-50 dark:text-zinc-300 dark:hover:bg-zinc-800">
              ...
              <Link href="/profile?tab=notifications" onClick={() => setMenuOpen(false)}
                className="flex items-center gap-3 px-4 py-2.5 text-sm text-zinc-700 transition-colors hover:bg-zinc-50 dark:text-zinc-300 dark:hover:bg-zinc-800">
                <svg className="h-4 w-4 text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>
                <span className="flex-1">Notifications</span>
                {unreadCount > 0 && (
                  <span className="rounded-full bg-indigo-600 px-1.5 py-0.5 text-[10px] font-bold leading-none text-white">{unreadCount}</span>
                )}
              </Link>
              <Link href="/profile?tab=settings" onClick={() => setMenuOpen(false)}
                className="flex items-center gap-3 px-4 py-2.5 text-sm text-zinc-700 transition-colors hover:bg-zinc-50 dark:text-zinc-300 dark:hover:bg-zinc-800">
