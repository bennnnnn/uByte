---
title: "Loops"
description: "Master Go's for loop — the only loop keyword you need for counting, iterating, and repeating."
order: 5
difficulty: "beginner"
estimatedMinutes: 12
---
...
- **for (classic)** — init; condition; post (e.g. count 1 to 5)
- **for (while-style)** — omit init/post; condition only
- **for range** — iterate over slices (index and value)
...
Unlike most languages that have `for`, `while`, and `do-while`, Go has **only one loop keyword**: `for`. But don't worry — it's incredibly versatile and covers every looping pattern you'll ever need.
...
Drop the init and post statements to get a "while" loop — just a condition:
...
> **Note:** The emoji `🎉` takes 4 bytes, so the index will skip from its position to +4. `range` handles multi-byte characters correctly.
...
Loops inside loops — useful for 2D data, grids, and combinations:
...
func main() {
    ...
    for i, task := range tasks {
        ...
        if task == "deploy" {
            fmt.Println("  ⚠️  Deploy step reached — stopping pipeline")
            break
        }
    }
...
Now you can repeat actions efficiently! Next, let's explore **arrays and slices** — Go's tools for working with ordered collections of data.
