...
- **Create a map** — `map[K]V{}`, literal with key: value
- **Check if key exists** — two-value form `v, ok := m[k]`
- **Delete a key** — `delete(m, key)`
- **Iterate over map** — `for k, v := range m`
- **Word count** — split string, count with map
