...
- **Define a struct** — `type Name struct { Field Type }`, struct literals
- **Methods (value receiver)** — `func (p Person) Greet() string`
- **Nested structs** — struct fields that are structs
- **Anonymous struct** — one-off `struct{ ... }{ ... }`
- **Pointer to struct** — `new(T)`, modifying fields through pointer
