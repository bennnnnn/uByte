...
- **Address and dereference** — `&x` (address of x), `*p` (value at p)
- **Modify via pointer** — function that takes `*int` and changes the value
- **Pointer to struct** — access fields through pointer (e.g. `p.Name`)
- **Swap with pointers** — swap two integers using a function with `*int` params
