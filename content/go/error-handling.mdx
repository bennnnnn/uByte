...
- **Return an error** — `(result, error)`; use `fmt.Errorf` when failing
- **Check an error** — `if err != nil { ... }`; handle or return
- **Custom error type** — struct with `Error() string` method
- **Wrap errors** — `fmt.Errorf("msg: %w", err)` for error chains
- **errors.Is** — check if wrapped error matches a target
