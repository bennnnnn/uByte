...
- **Slice literal** — declare and print a slice of strings
- **append** — add elements; returns new slice; use `len()`
- **Slicing** — `s[low:high]` to get a sub-slice
- **Range to sum** — `for _, v := range slice` and accumulate
- **2D slice** — slice of slices; index with `grid[i][j]`
