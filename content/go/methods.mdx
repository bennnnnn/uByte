...
- **Value receiver** — method on struct; gets a copy (e.g. Area)
- **Pointer receiver** — method that modifies the struct (e.g. Increment)
- **Stringer** — `String() string` for custom fmt.Println output
- **Method chaining** — return receiver for fluent API (e.g. Builder)
